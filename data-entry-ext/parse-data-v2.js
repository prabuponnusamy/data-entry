    ...
    for (let i = 0; i < maxLen; i++) {
        ...
        tableHTML += `<tr style="display:${show ? 'table-row' : 'none'}"><td>${i + 1} <button class="delete-row-btn">Delete</button></td>
            <td>
                ...
                <textarea id="original-msg-${i}" name="original-msg" class="original-msg ${hasWinningNumber.length > 0 ? 'winning-ticket' : ''}" data-idx="${i}" rows="${inputGroups[i]?.length || 1}">${inputMsg}</textarea>
                ${imgHtml}
            </td>
            <td>
